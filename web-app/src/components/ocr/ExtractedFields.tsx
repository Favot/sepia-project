...
interface ExtractedFieldsProps {
  result: Pick<OcrResult, "numeroSiret" | "numeroEngagement" | "codeService" | "extractionMethod">;
}

export function ExtractedFields({ result }: ExtractedFieldsProps) {
  const { numeroSiret, numeroEngagement, codeService, extractionMethod } = result;
  ...
  return (
    <div className="mt-6">
      ...
      <div className="space-y-3">
        ...
        {codeService && <FieldRow label="Code de service" value={codeService} />}
      </div>
    </div>
  );
}
...
function FieldRow({ label, value }: { label: string; value: string | null }) {
  ...
  return (
    <div className="flex items-center gap-3 p-4 bg-card rounded border border-border">
      <span className="text-sm font-medium text-muted-foreground min-w-[180px]">{label}</span>
      <span className="text-base font-mono font-semibold text-foreground flex-1">{value}</span>
      <button
        onClick={handleCopy}
